...
def _synonyms(species):
    ...
    syn = pd.read_csv(path("data/species_synonyms.csv"),
                    comment="#").fillna("")

    syn = _split_explode(syn, "synonyms")

    # Find where "species" appears exactly in either the index or the "synonyms" column
    syn = syn[(syn["species"].str.lower() == species.lower()) | \
              (syn["synonyms"].str.lower() == species.lower())]
...
def _scale_graph(species):
    ...
    data = pd.read_csv(path("data/convert_functions.csv"),
                       comment="#").fillna("")
    ...
    for i in range(len(data)):
        ...
        if data.iloc[i]['y=f(x)']:
            ...
            G.add_edge(data.iloc[i]['scale_x'].lower(),
                       data.iloc[i]['scale_y'].lower(),
                       f=f)
            # store the inverse function
            G.add_edge(data.iloc[i]['scale_y'].lower(),
            ...
            if data.iloc[i]['t_start']:
                ...
                f = Piecewise((ft, (t >= t_start) & (t <= t_end)),
                              (t_else, True))
...
def convert(c, species, scale_original, scale_new):
    ...
    try:
        ...
        path = nx.shortest_path(G, source=scale_original.lower(),
                                target=scale_new.lower())
        ...
        for i in range(len(path)-1):
            ...
            if len(f_edge.free_symbols) > 1:
                raise ValueError(f"Conversion function {f_edge} contains multiple inputs. Only one input is allowed at the moment")
